#include "tm4c123gh6pm.h"
#include "TExaS.h"
#include "UserInterface.h"
#include "Nokia5110.h"
#include "InputHandler.h"
#include "ConnectFour.h"


// *************************** Images ***************************

const unsigned char Logo[] ={
 ,

} ;
const unsigned char Cell[] ={
 ,

};



const unsigned char X[] ={
 ,

};



const unsigned char O[] ={
 ,

};

const unsigned char Oinverted[] ={
 ,

};

const unsigned char Xinverted[] ={
 ,

};







//*************************Global Var***************************
int PlayerTurn ;
char PlayerSeed ;
int PlayerRole;
int BoxLineWidth = 1 ;
int BoardHeight =48;
int PaddingY = 2 ;
int BoxWidth = 6;
int PaddingX = 18 ;
char gameBoard[ROWS][COLS];
int deltaX, deltaY, row, column;

void NokiaInit (void)
{
	int i,j;
	TExaS_Init(SSI0_Real_Nokia5110_Scope);  // set system clock to 80 MHz
  Nokia5110_Init();
	for (i=0;i<7;i++)
	{
		for (j=0;j<6;j++)
		{
			gameBoard[i][j] = 0;
		}
	}
}

void welcomeScene (void)
{
	Nokia5110_ClearBuffer();
	Nokia5110_DisplayBuffer();
	Nokia5110_PrintBMP(17, 38,Logo, 0);
	Nokia5110_DisplayBuffer();
}
int selectModeScene (void)
{
	Nokia5110_Clear();
	Nokia5110_SetCursor (2,1);
	Nokia5110_OutString ("1.Single.");
	Nokia5110_SetCursor (2,3);
	Nokia5110_OutString ("2.Multi.");
	do{
		PlayerTurn = getInputKey();
	}while(PlayerTurn > '2' || PlayerTurn < '1');
	return PlayerTurn - '0';
}

char selectSeedScene(void)
{
	Nokia5110_Clear();
	Nokia5110_SetCursor (0,1);
	Nokia5110_OutString ("Press:");
	Nokia5110_SetCursor (0,2);
	Nokia5110_OutString ("*.player 1 .");
	Nokia5110_SetCursor (0,3);
	Nokia5110_OutString ("0.player 2 .");
	do{
		PlayerSeed = getInputKey();
	}while(PlayerSeed != '*' && PlayerSeed != '0');
	return (PlayerSeed == '*' ? 'X' : 'O');
}

void Delay100ms(unsigned long count){unsigned long volatile time;
  while(count>0){
    time = 727240;  // 0.1sec at 80 MHz
    while(time){
	  	time--;
    }
    count--;
  }
}

void waitingScene(void)
{
	int i;
	
	Nokia5110_Clear();
	Nokia5110_SetCursor(1,2);
	Nokia5110_OutString ("Waiting");
	for (i=0;i<3;i=i+1)
	{
		Delay100ms(1);
		Nokia5110_OutString (".");
	}
}
int MSScene (void)
{
	Nokia5110_Clear();
	Nokia5110_SetCursor (0,0);
	Nokia5110_OutString ("Press:");
	Nokia5110_SetCursor (1,2);
	Nokia5110_OutString ("Master (1)");
	Nokia5110_SetCursor (1,3);
	Nokia5110_OutString ("Slave  (2)");
	do{
		PlayerRole = getInputKey();
	}while(PlayerRole > '2' || PlayerRole < '1');
	return PlayerRole ;
}

void displaySlaveSeed (unsigned short turn) 
{
	char t='0'+turn;
	Nokia5110_Clear();
	Nokia5110_SetCursor (2,1);
	Nokia5110_OutString ("You are");
	Nokia5110_SetCursor (2,3);
	Nokia5110_OutString ("PLAYER"); 
	Nokia5110_OutChar(t);
}

void stateScene(enum GameState gameState, char seed)
{
	unsigned short state = gameState;
	char* statement;

	Nokia5110_Clear();
	if ((state==0&& seed =='X') || (state==1&& seed =='O'))
	{
		Nokia5110_SetCursor (2,2);
		statement = "YOU WIN";
	}
	else if  ((state==0&& seed =='O') || (state==1&& seed =='X'))
	{
		Nokia5110_SetCursor (1,2);
		statement = "YOU  LOSE";
	}
	else if (state==2)
	{
		Nokia5110_SetCursor (4,2);
		statement = "TIE";
	}
	
	Nokia5110_OutString (statement);
}

void drawSeed(char seed, int col ,int row)
{
	const unsigned char *Seed ;
	if (seed=='X'||seed=='x') Seed=X;
	else if (seed=='O'||seed=='o') Seed=O;
	if (row > 5 || col < 0 ||col >6) //invalid
		return ;
	gameBoard[row][col] = seed;
	Nokia5110_PrintBMP(PaddingX+BoxLineWidth*(col+1)+BoxWidth*col,(BoardHeight-PaddingY)-BoxLineWidth*(row+1)-BoxWidth*row,Seed,0);
	Nokia5110_DisplayBuffer();

}

void drawInvertedSeed(char seed, int col ,int row)
{
	const unsigned char *Seed ;
	if (seed=='X'||seed=='x') Seed=Xinverted;
	else if (seed=='O'||seed=='o') Seed=Oinverted;
	if (row > 5 || col < 0 ||col >6) //invalid
		return ;
	
	Nokia5110_PrintBMP(PaddingX+BoxLineWidth*(col+1)+BoxWidth*col,(BoardHeight-PaddingY)-BoxLineWidth*(row+1)-BoxWidth*row,Seed,0);
	Nokia5110_DisplayBuffer();

}
void connectingScene (void)
{
	int i;
	
	Nokia5110_Clear();
	Nokia5110_SetCursor(0,2);
	Nokia5110_OutString ("Connecting");
	for (i=0;i<2;i=i+1)
	{
		Delay100ms(1);
		Nokia5110_OutString (".");
	}

}


void drawEmptyGrid(void)
{
	int i,j;
	Nokia5110_ClearBuffer();
	Nokia5110_DisplayBuffer();
	for (i=0;i<7;i++)
	{
		for (j=0;j<6;j++)
		{
			Nokia5110_PrintBMP(PaddingX+BoxLineWidth*i+BoxWidth*i,(BoardHeight-PaddingY)-j*BoxLineWidth-j*BoxWidth,Cell, 0);
		}
	}

	Nokia5110_DisplayBuffer();

}

void winGrid ()
{
	int i ,j;
	
	Nokia5110_ClearBuffer();
	Nokia5110_DisplayBuffer();
	fullGrid();

	for(i=0;i<4;i++)
	{
		for (j=0;j<4;j++)
		{
			drawInvertedSeed(gameBoard[row][column],column,row);
			row+=deltaY;
			column+=deltaX;
		}
	}

}
void fullGrid (void)
{
	int i ,j;
	const unsigned char *Seed ;
	Nokia5110_ClearBuffer();
	Nokia5110_DisplayBuffer();
	drawEmptyGrid();
	for (i=0;i<ROWS;i++)
	{
		for (j=0;j<COLS;j++)
		{
			if(gameBoard[i][j]=='X')  drawSeed('X',j,i);
			else if (gameBoard[i][j]=='O')  drawSeed('O',j,i);
		}
	}

}

 void winScene (void)
 {
	 int cnt =5 ;
	 
	 while(cnt--)
	 {
		
		winGrid();
		Delay100ms(5);
		fullGrid();
		
		
		Nokia5110_ClearBuffer();
		Nokia5110_DisplayBuffer();
	 }
		
 }
 
void genPoint(void){
	int i, j;
  for(i=0; i<ROWS; i++){
    for(j=0; j<COLS; j++){
      if(j<4 && gameBoard[i][j]==gameBoard[i][j+1] && gameBoard[i][j]==gameBoard[i][j+2] && gameBoard[i][j]==gameBoard[i][j+3]){
        row = i;
				column = j;
				deltaX = 0;
				deltaY = 1;
				return;
      }
      if(i<3 && gameBoard[i][j]==gameBoard[i+1][j] && gameBoard[i][j]==gameBoard[i+2][j] && gameBoard[i][j]==gameBoard[i+3][j]){
        row = i;
				column = j;
				deltaX = 1;
				deltaY = 0;
				return;
      }
      if(i<3 && j<4 && gameBoard[i][j]==gameBoard[i+1][j+1] && gameBoard[i][j]==gameBoard[i+2][j+2] && gameBoard[i][j]==gameBoard[i+3][j+3]){
        row = i;
				column = j;
				deltaX = 1;
				deltaY = 1;
				return;
      }
      if(i>2 && j<4 && gameBoard[i][j]==gameBoard[i-1][j+1] && gameBoard[i][j]==gameBoard[i-2][j+2] && gameBoard[i][j]==gameBoard[i-3][j+3]){
        row = i;
				column = j;
				deltaX = -1;
				deltaY = 1;
				return;
      }
    }
  }
}
